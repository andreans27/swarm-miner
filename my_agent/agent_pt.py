...
# --- consistency-gate fix --------------------------------------------------
# The CNN's position head is accurate on every map (cm-level when the pad is in
# view), but its confidence head (visibility_prob) is dead on a large fraction
# of mountain/warehouse seeds, so the champion's `pr > 0.8` gate discards those
# accurate positions. We additionally accept a detection when the RAW predicted
# world position is stable across consecutive frames (a true pad gives a steady
# world point; phantoms / no-pad jump around). Additive: never overrides the
# existing pr>0.8 path, only rescues cases where confidence is wrongly low.
CONSIST_N = 6           # frames of history used for the stability test
CONSIST_SPREAD = 1.0    # max deviation (m) from the window mean to count as stable
CONSIST_NEAR = 25.0     # predicted pad must be within this of the search center (m)
